...
class c_CloudAdd extends CI_Controller {
			...
			if($_POST)
			{
					...
					$this->load->library('upload',$config);
					  ...
					  if(!$this->upload->do_upload("gambar"))
					  {
					    ...
					    echo"<script> alert('Upload gagal!');window.location.href='".base_url('c_CloudAdd')."'</script>";
					    exit();
					  }
					$file = $this->upload->data();
					$data['dt_gambar'] = $file['file_name'];
					$this->load->model('m_CloudAdd');
					$this->m_CloudAdd->insert($data);
			}
					redirect('c_dashboard');
		 }
		public function index(){
			if($this->session->userdata('level')=="superadmin" || $this->session->userdata('level')=="admin"){
				$this->load->view('Operator/v_cloud-add');
			}
			else{
